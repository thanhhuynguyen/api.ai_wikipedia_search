...
def distanceRequest(req):
    ...
    yql_query = makeDistanceQuery(req)
    if yql_query is None:
        return {}
    
    payload = {
        'origins' : '|'.join(yql_query[0]),
        'destinations' : '|'.join(yql_query[1]), 
        'mode' : 'driving',
        'api_key' : api_key
    }
...
def makeDistanceResult(data):
    for isrc, src in enumerate(data['origin_addresses']):
        for idst, dst in enumerate(data['destination_addresses']):
            ...
            if cell['status'] == 'OK':
                speech = 'i got distance between {} and {} is {}, if you driving via car, you will loss {}.'.format(src, dst, cell['distance']['text'], cell['duration']['text']))
            else:
